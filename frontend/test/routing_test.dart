...
void main() {
  ...
  group('report tabs in the URL', () {
    test('the default panel keeps the plain project link', () {
      // /projects/<id> is what people paste; it should not grow a suffix just
      // because the report has panels now.
      expect(const AppRoute.report('p1').location, '/projects/p1');
      expect(
        const AppRoute.report('p1', tab: ReportTab.packages).location,
        '/projects/p1',
      );
    });

    test('every other panel is linkable', () {
      for (final tab in ReportTab.values) {
        final route = AppRoute.report('p1', tab: tab);
        expect(AppRoute.parse(route.location), route, reason: tab.name);
      }
    });

    test('names the panel in the path', () {
      expect(
        const AppRoute.report('p1', tab: ReportTab.advisories).location,
        '/projects/p1/advisories',
      );
    });

    test('an unrecognised panel opens the report rather than failing', () {
      expect(
        AppRoute.parse('/projects/p1/nonsense'),
        const AppRoute.report('p1'),
      );
    });

    test('a project and a panel of it are different routes', () {
      expect(
        const AppRoute.report('p1'),
        isNot(const AppRoute.report('p1', tab: ReportTab.tree)),
      );
    });

    test('switching panels replaces rather than stacking history', () {
      // Four clicks around a report should not take four presses to leave.
      final d = AppRouterDelegate(api: ApiClient())
        ..go(const AppRoute.report('p1'))
        ..showTab(ReportTab.advisories)
        ..showTab(ReportTab.licenses);

      expect(d.stack.length, 2);
      expect(
        d.currentConfiguration,
        const AppRoute.report('p1', tab: ReportTab.licenses),
      );
    });

    test('showTab does nothing when no report is open', () {
      final d = AppRouterDelegate(api: ApiClient())..showTab(ReportTab.tree);
